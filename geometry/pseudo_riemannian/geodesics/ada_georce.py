...
class AdaGEORCE(ABC):
    ...
    def update_scheme(self, 
        ...
        rhs = jnp.sum(vmap(self.sgt_inv, in_axes=(0,0,None))(zt[:-1], sg_cumsum, batch), axis=0)\
            +2.0*self.diff
        
        muT = -self.sgt_sum_inv(self.z0, zt, rhs)
    ...
    def adaptive_default(self,
        ...
        zt_k2 = (1.-self.beta1)*zt_k2+self.beta1*zt_k1
    ...
    def __call__(self, 
        ...
        self.PM = IndicatorManifold(M=self.M, 
                                    extrinsic_batch_size=self.M.emb_dim, 
                                    intrinsic_batch_size=self.batch_size,
                                    seed=self.seed)
